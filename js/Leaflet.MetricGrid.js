...
L.MetricGrid = L.Layer.extend({
	...
	options: {
		...
		opacity: 1,															// 0.7
		weight: 1,                                           //0.5 use 2 for best results, else label rub-out is less good (antialiased pixels)
		color: "#000000", // #00f #005f00
		font: "italic 14px Sans-serif",     // bold 16px Verdana normal
		density: 1,
		...
		minZoom: 15,                        // 1/25000  // minimum zoom at which grid is drawn : 4
		maxZoom: 16									// 1/12500
	},
	...
	_formatEastOrNorth(n, spacing) {
		...
		var r;
		var h = Math.floor(n / 1000);
/*		n = n % 100000; // metres within 100km square = 100000
				// MD tous les chiffres
...
*/		
/*
		// prepend hundreds of km in subscript // MD même taille pour tous les chiffres
		if (this.options.showAxis100km) {
				var hs = h.toString();
				var i;
				for(i = (hs.length-1); i >= 0; i--) {
					r = String.fromCharCode(hs.charCodeAt(i) + 0) + r; // String.fromCharCode(hs.charCodeAt(i) + 8272) + r
				}
		}
//r = n;
*/
...
alert("x= "+x+" : pts[0].x="+pts[0].x+" pts[0].y="+pts[0].y + " || "+" pts[max].x="+pts[max].x+" pts[max].y="+pts[max].y);
*/
					...
					for (x = grdWx; x <= grdEx; x += d) {
						for (y = grdSy; y <= grdNy; y += d) {
								...
								if ((s.x > 0) && (s.y < hh) && (x < this.options.bounds[1][0])) {
									...
									// MD : ajout de la zone et placement en haut sur fond blanc
									
									var eStr = '('+this.options.zone+') '+ this._format_eastings(x, d);
...
//									ctx.clearRect(s.x,0,txtWidth,txtHeight * 1.2);
									// MD fond blanc pour le label en haut
									ctx.fillStyle = "white";
									ctx.fillRect(s.x +0,canvas.height - txtHeight - 6, txtWidth+6, canvas.height - txtHeight - 6);
									ctx.fillStyle = "blue";
									ctx.fillText(eStr, s.x+3 , canvas.height -  3);
									break;
								}
						}
					}
					...
					for (y = grdSy; y <= grdNy; y += d) {
						for (x = grdWx; x <= grdEx; x += d) {
								...
								if ((s.x > 0) && (s.y < hh) && (y < this.options.bounds[1][1])) {
									...
									// MD ajout de la bande et placement à gauche sur fond blanc
									var bandNames = 'CDEFGHJKLMNPQRSTUVWXX';
									var band = bandNames.charAt(Math.floor(ll[1]/8.0+10.0));
									
									var nStr = '('+band+') '+ this._format_northings(y, d);
...
//									ctx.fillText(nStr, s.x, s.y + (txtHeight / 2));
									// MD fond blanc pour le label en haut
									ctx.fillStyle = "white";
									ctx.fillRect(0, s.y - txtHeight - 6, txtWidth+6, txtHeight + 6);
									ctx.fillStyle = "blue";
									
									ctx.fillText(nStr, 3, s.y-3);
									break;
								}
						}
					}
