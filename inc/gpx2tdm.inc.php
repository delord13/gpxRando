...
////////////////////////////////////////////////////////////////////////////////
function openRouteService($coordParking, $tollways) { // tollways true false
// PB : limite de 40 requêtes par minute : pas assez pour récupérer la commune des 2 itinéraires
	...
	$ch = curl_init();
	$opt = "https://api.openrouteservice.org/directions?api_key=$cle&coordinates=".LONRDV.",".LATRDV."%7C$coordParking&format=gpx&profile=driving-car&language=fr&geometry=true&instructions=true&roundabout_exits=true$option";
	...
	if (substr($reponse,0,8)=="{\"error\"") {
		...
		$openRoute['temps'] = "";
		$openRoute['gpxTrk'] = "";
		$openRoute['itineraire'] = "";
		$openRoute['modeVisualiser'] = "";
		return $openRoute;
	}
	
	$gpxRte = simplexml_load_string($reponse);
	$km = $gpxRte->rte->extensions->distance/1000;
	$openRoute['km'] =floor(($km+2.5)/5)*5; 

	// temps
	$openRoute['temps'] = $gpxRte->rte->extensions->duration;
	...
	foreach ($gpxRte->rte->rtept as $rtept) {
		$lat =  (string)(round((float)$rtept['lat']*100000)/100000);
		$lon = (string)(round((float)$rtept['lon']*1000000)/1000000);
	...
	foreach ($gpxRte->rte->rtept as $rtept) {
		if ($i>0) {
			$d = distance($latPrec, $lonPrec, (float)$rtept['lat'], (float)$rtept['lon']);
			$distanceCumulee += $d;
			$lonPrec = (float)$rtept['lon'];
			$latPrec = (float)$rtept['lat'];
		}
		else {
			$distanceCumulee = 0;
			$lonPrec = (float)$rtept['lon'];
		...
		$newName = (string)$rtept->name;
		$newDesc = (string)$rtept->desc;
		$newDesc = str_replace("Radlofzell","Radolfzell",$newDesc);
		$newDesc = str_replace("Entrez le","Entrez dans le",$newDesc);
		$newDesc = str_replace("Tournez gauche","Tournez à gauche",$newDesc);
		$newDesc = str_replace("Tournez droit","Tournez à droite",$newDesc);
		$newDesc = str_replace("Tournez Léger","Tournez légèrement",$newDesc);
		$newDesc = str_replace("Tournez Pointu","Tournez fortement",$newDesc);
		$newDesc = str_replace("Tournez à le","Tournez au",$newDesc);
		$newDesc = str_replace("Départmentale","Départementale",$newDesc);
		$newDesc = str_replace("on the left","sur la gauche",$instruction);
		$newDesc = str_replace("on the right","sur la droite",$instruction);

		if (strcmp($newName, $name) !== 0 || strcmp($newDesc, $desc) !== 0) {
		...
		// recherche de la commune voir PB
         ...
         // construction de la ligne itinéraire
			$itineraire .= "\t".$distanceKm." km\t".$locality." ".$newDesc.PHP_EOL;
			$name = $newName;
			$desc = $newDesc;
		}
	}
	...
	$minlat = $gpxRte->bounds['minLat'];
	$minlon = $gpxRte->bounds['minlon'];
	$maxlat = $gpxRte->bounds['maxLat'];
	$maxlon = $gpxRte->bounds['maxlon'];
